...
- If yes, run:
  ```bash
  pnpm create sanity@latest -- --template clean --typescript
  ```
...
**If MCP fails:** Remind them to run `pnpx sanity schema deploy` first.

### MCP Setup (If Not Configured)

**Quick start via Sanity CLI:**
```bash
pnpx sanity@latest mcp configure
```
...
```bash
pnpx sanity@latest mcp configure  # Configure MCP for your editor
pnpx sanity dev                   # Start Studio locally
pnpx sanity schema deploy         # Deploy schema (required for MCP!)
pnpx sanity deploy                # Deploy Studio to Sanity hosting
pnpx sanity manage                # Open project settings
pnpm run typegen                  # Generate TypeScript types
```
