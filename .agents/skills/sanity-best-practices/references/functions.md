...
> **Experimental feature**: APIs may change. Always use `pnpx sanity@latest`.
...
```bash
pnpx sanity@latest blueprints init . \
  --type ts \
...
```bash
pnpx sanity@latest blueprints add function \
  --name my-function \
...
# CLI testing
pnpx sanity@latest functions test my-function \
  --dataset production \
  --with-user-token
...
```bash
pnpx sanity@latest functions logs my-function
pnpx sanity@latest functions logs my-function --watch
```
...
2. CLI: `pnpx sanity functions env add my-function MY_VAR my-value`
3. Local testing: `MY_VAR=value pnpx sanity functions test my-function`
...
Set the env var: `pnpx sanity functions env add deploy-hook DEPLOY_HOOK_URL https://...`
